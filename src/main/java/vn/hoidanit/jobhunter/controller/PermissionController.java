...
public class PermissionController {
    ...
    public ResponseEntity<Permission> update(@Valid @RequestBody Permission p) throws IdInvalidException {
        Permission permission = this.permissionService.fetchById(p.getId());
        if (permission == null) {
    ...
    @ApiMessage("fetch all permission with paginate")
    public ResponseEntity<ResultPaginationDTO> fetchAll(@Filter Specification<Permission> spec,
            Pageable pageable) {
